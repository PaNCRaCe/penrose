...
from libFlechesCerfsVolants import soleil_start_demis, etoile_start_demis, cut_dfl, cut_dcv
from libLosanges import figure_start_demis, cut_dGL, cut_dpL
...
rendu1 = {
          "color": "#007fff", #"070707", #"#0090fe", # "#007fff"
          "texturer": True,
          "ect_texture": 20.0,
          "teinte_aleatoire" : True,
          "ect_teinte": 20.0,
...
rendu2 = {
          "color" : "#ff8000", #"#f8f8f8", #"#7fbeff", # "#ff8000"
          "texturer" : True,
          "ect_texture": 10.0,
          "teinte_aleatoire" : True,
          "ect_teinte": 5.0,
...
# optimiser OK
# todo : multiprocesser OK
# todo : librairies OK
# decoupage des demis et appariement a la derniere etape uniquement
...
def draw():
    print "...PROCESSING : appariement des demi figures"
    t0 = time.time()
    ### 1 PROCESS
    polys_1 = match_demis(demis_1)
    polys_2 = match_demis(demis_2)
    ###
...
if __name__ == "__main__":
    ...
    #    GL, pL = soleil_start(rotation)
        demis_1, demis_2 = figure_start_demis(rotation) # Gros et petits losanges
    ...
    #    GL, pL = soleil_start(rotation)
        if figure_depart == "soleil":
            demis_1, demis_2 = soleil_start_demis(rotation) # cerfs-volants et fleches
        elif figure_depart == "etoile":
            demis_1, demis_2 = etoile_start_demis(rotation) # cerfs-volants et fleches
        else:
...
#        cutted_pl = [cut_pL(pp) for pp in pL]
        ...
        if penrose_type == 3: # "losanges":
            cut_1 = partial(cut_dGL, flag_reverse)
            cut_2 = partial(cut_dpL, flag_reverse)

        try:
            cutted_1 = pool_cut.map(cut_1, demis_1)
            cutted_2 = pool_cut.map(cut_2, demis_2)
...
#        import pdb; pdb.set_trace()
        ...
        try:
            demis_1 = np.vstack([item[dtype1] for item in cutted_1 + cutted_2 if dtype1 in item.keys()])
        except ValueError: # tous vides
            demis_1 = np.array([])

        try:
            demis_2 = np.vstack([item[dtype2] for item in cutted_1 + cutted_2 if dtype2 in item.keys()])
        except ValueError: # tous vides
            demis_2 = np.array([])
...
#        print float((demis_1[0][0] - demis_1[0][2]).sqr_norm()), float((demis_2[0][0] - demis_2[0][2]).sqr_norm())
        
        if penrose_type == 3: # "losanges":
            try:
                buffered_view = view.buffer(fractions.math.sqrt((demis_1[0][0] - demis_1[0][2]).sqr_norm())) # ajout d'un buffer pour pouvoir matcher les losanges en bordure
            except:
                buffered_view = view.buffer(fractions.math.sqrt((demis_2[0][0] - demis_2[0][2]).sqr_norm())) # ajout d'un buffer pour pouvoir matcher les losanges en bordure
...
#        GL = subset_view(GL, buffered_view)
        ...
        p_conn_2, c_conn_2 = Pipe()
        try:
            p1 = Process(target=subset_view, args=(demis_1, buffered_view, c_conn_1))
            p1.start()
            p2 = Process(target=subset_view, args=(demis_2, buffered_view, c_conn_2))
            p2.start()
        except KeyboardInterrupt, e:
            p1.terminate()
            p2.terminate()
        ...
        demis_1 = np.array(p_conn_1.recv())
        demis_2 = np.array(p_conn_2.recv())
        p1.join()
        p2.join()
